...
# ---------------------------- CONFIGURATION ----------------------------
GEMINI_API_KEY =  os.getenv("GENAI_API_KEY") # Replace with your Gemini API key
...
if section == "Upload & Predict":
    ...
    if uploaded:
        try:
            ...
            # Check if Master Category confidence is low (likely not a bottle)
            if predictions["master"]["confidence"] < 0.5:
                st.warning("⚠️ The uploaded image may not be a bottle. Please upload a bottle image.")
            else:
...
# ---------------------------- SECTION: COMPARE BOTTLES ----------------------------
    ...
    if uploaded1 and uploaded2:
        try:
            ...
            # Check for non-bottle images
            if pred1["master"]["confidence"] < 0.5:
                st.warning("⚠️ Bottle 1 may not be a bottle image. Please upload a proper bottle image.")
            if pred2["master"]["confidence"] < 0.5:
                st.warning("⚠️ Bottle 2 may not be a bottle image. Please upload a proper bottle image.")

            if pred1["master"]["confidence"] >= 0.5 and pred2["master"]["confidence"] >= 0.5:
...
# ---------------------------- FOOTER ----------------------------
st.markdown("---")
st.caption("AI-Powered Dashboard by Deepak H R • © 2025")
